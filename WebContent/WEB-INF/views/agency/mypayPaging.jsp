...
<%
	...
	***********************************/
	...
	if(request.getParameter("pageSize") != null){
		...
		System.out.println("boardPaging : pageSize >>> : " + pageSize);
	}
	
	if(request.getParameter("groupSize") != null){
		groupSize = Integer.parseInt(request.getParameter("groupSize"));//5
		System.out.println("boardPaging : groupSize >>> : " + groupSize);
	}
	
	if(request.getParameter("curPage") != null){
		curPage = Integer.parseInt(request.getParameter("curPage"));//1
		System.out.println("boardPaging : curPage >>> : " + curPage);
		
	}
	
	if(request.getParameter("totalCount") != null){
		totalCount = Integer.parseInt(request.getParameter("totalCount"));//400
		System.out.println("boardPaging : totalCount >>> : " + totalCount);
	}
	
	if(request.getParameter("keyFilter") != null) {
		keyFilter = request.getParameter("keyFilter");
		System.out.println("boardPaging : keyFilter >>> : " + keyFilter);
	}
	
	if(request.getParameter("keyword") != null) {
		keyword = request.getParameter("keyword");
		System.out.println("boardPaging : keyword >>> : " + keyword);
	}
	
	if(request.getParameter("startDate") != null) {
		startDate = request.getParameter("startDate");
		System.out.println("boardPaging : startDate >>> : " + startDate);
	}
	
	if(request.getParameter("endDate") != null) {
		endDate = request.getParameter("endDate");
		System.out.println("boardPaging : endDate >>> : " + endDate);
	}
...
%>
	<a href="<%=url%>?<%=str%>curPage=1&keyfilter=<%=keyFilter%>&keyword=<%=keyword%>&startdate=<%=startDate%>&enddate=<%=endDate%>">◁◁</a>&nbsp;&nbsp;&nbsp;
	<a href="<%=url%>?<%=str%>curPage=<%=linkPage%>&keyfilter=<%=keyFilter%>&keyword=<%=keyword%>&startdate=<%=startDate%>&enddate=<%=endDate%>">◀</a>&nbsp;&nbsp;&nbsp;
<%
	}else{
%>
		◁◁&nbsp;&nbsp;&nbsp;◀&nbsp;&nbsp;&nbsp;
...
<%
			...
			//[2][3][4][5]
%>
		[<a href="<%=url%>?<%=str%>curPage=<%=linkPage%>&keyfilter=<%=keyFilter%>&keyword=<%=keyword%>&startdate=<%=startDate%>&enddate=<%=endDate%>">
		<%=linkPage%></a>]&nbsp;
<%
		}
...
%>
	<a href="<%=url%>?<%=str%>curPage=<%=linkPage%>&keyfilter=<%=keyFilter%>&keyword=<%=keyword%>&startdate=<%=startDate%>&enddate=<%=endDate%>">▶</a>&nbsp;&nbsp;&nbsp;
	<a href="<%=url%>?<%=str%>curPage=<%=pageCount%>&keyfilter=<%=keyFilter%>&keyword=<%=keyword%>&startdate=<%=startDate%>&enddate=<%=endDate%>">▷▷</a>&nbsp;&nbsp;&nbsp;
<%
	}else{
		System.out.println("다음그룹이 있는 경우 linkPage >>> : " + linkPage);
		System.out.println("다음그룹이 있는 경우pageCount >>> : " + pageCount);
